...
{
	public class CongeledCorruptProjectile : ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			for (int i = 0; i < list.Count - 1; i++) {
				...
				float rotation = diff.ToRotation() - MathHelper.Pi; // This projectile's sprite faces down, so PiOver2 is used to correct rotation.
		...
		public override void AI() {
			...
			if (Timer == swingTime / 2) {
				...
				Projectile.FillWhipControlPoints(Projectile, points); //don't forget to call this
				SoundEngine.PlaySound(SoundID.Item153, points[points.Count - 1]);
			}
			//Dust is drawn here, only after halfway for aesthetic reasons, make sure you call FillWhipPoints Earlier
			if (Timer >= swingTime / 2) { 
				List<Vector2> points = Projectile.WhipPointsForCollision;
				Dust.NewDust(points[points.Count - 1], Projectile.width, Projectile.height, DustID.CorruptTorch, Projectile.velocity.X, Projectile.velocity.Y, 25, default(Color), .7f);
			}
		}
	}
}
